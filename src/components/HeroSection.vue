...
function onScroll() {
  ...
  requestAnimationFrame(() => {
    ...
    if (layerA.value) layerA.value.style.transform = `translate3d(0, ${y * 0.12}px, 0)`
    if (layerB.value) layerB.value.style.transform = `translate3d(0, ${y * 0.22}px, 0) rotate(${y * 0.03}deg)`
    ticking = false
  })
}
...
onMounted(() => {
  ...
  if (parallaxEnabled) window.addEventListener('scroll', onScroll, { passive: true })
})
...
<template>
  <section id="top" class="hero">
    <div class="hero-bg-photo">
      <PhotoPlaceholder label="Фон: ремонтный цех, разобранная АКПП на столе" ratio="16 / 10" />
    </div>
    ...
    <div class="container hero-content">
      ...
      <h1 class="hero-title">
        Ремонт АКПП, DSG, вариаторов, редукторов и раздаток.
        <span class="text-gold-shimmer">Для автомобилей и яхт.</span>
      </h1>
      <p class="hero-subtitle">
        От замены масла до капитального ремонта. Мехатроники, гидротрансформаторы, блоки клапанов,
        сцепление DSG. Работаем с гарантией до 2 лет.
      </p>

      <div class="hero-actions">
        <a href="#contacts" class="btn btn-gold">Получить консультацию мастера</a>
...
.hero::after {
  ...
  position: absolute;
  inset: 0;
  background: radial-gradient(60% 60% at 20% 20%, rgba(212, 168, 67, 0.12), transparent 70%),
